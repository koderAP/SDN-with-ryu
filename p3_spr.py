...
class ShortestPathSwitch(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.graph = {}       # {dpid: {neighbor_dpid: delay}}
    ...
    def build_graph(self):
        """Build the network graph with measured delays."""
        self.graph.clear()
        for src in self.adjacency:
            self.graph[src] = {}
            for dst in self.adjacency[src]:
                if (src, dst) in self.link_delays:
                    delay = self.link_delays[(src, dst)]
                else:
                    delay = 1  # Default delay if not measured yet
                self.graph[src][dst] = delay
        ...
        if src_dpid == dst_dpid:
            ...
            while node != src_dpid:
                if node is None:
                    return []
                path.insert(0, node)
                node = self.paths[src_dpid][node]
            path.insert(0, src_dpid)
        return path

    def install_path(self, path, src_mac, dst_mac):
        """Install flow entries along the path in both directions."""
        ...
        dst_out_port = self.mac_to_port[dst_dp.id][dst_mac]
        ...
        src_dp = self.datapaths[path[0]]
        src_out_port = self.mac_to_port[src_dp.id][src_mac]
        match = src_dp.ofproto_parser.OFPMatch(eth_src=dst_mac, eth_dst=src_mac)
        actions = [src_dp.ofproto_parser.OFPActionOutput(src_out_port)]
        self.add_flow(src_dp, 1, match, actions)
